...
struct DyteAudioPickerCellModel: PickerCellModel {
    let name: String
...
struct DytePickerModel<CellModel:PickerCellModel>: PickerModel {
    let title: String
    let selectedIndex: UInt
    let cells: [CellModel]
}

class DyteCustomPickerView<Model: PickerModel> : UIView, UIPickerViewDelegate, UIPickerViewDataSource {
    
    static func show(model: Model, on view: UIView) -> DyteCustomPickerView {
        let pickerView =   DyteCustomPickerView(heading: model.title, list: model.cells, selectedIndex: model.selectedIndex)
