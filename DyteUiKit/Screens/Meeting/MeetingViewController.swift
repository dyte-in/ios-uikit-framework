...
class DyteParticipantTileContainerView : UIView {
    ...
    func setParticipant(meeting: DyteMobileClient, participant: DyteJoinedMeetingParticipant) {
        prepareForReuse()
        let tile = DyteParticipantTileView(mobileClient: meeting, participant: participant, isForLocalUser: false)
        self.tileView = tile
...
public class MeetingViewController: UIViewController {
   ...
   private var gridView: GridView<DyteParticipantTileContainerView>!
    ...
    private func setUpReconnection() {
        self.viewModel.dyteSelfListner.observeMeetingReconnectionState { state in
            switch state {
            case .failed:
                ...
                UIUTility.displayAlert(alertTitle: "ConnectionLost!", message: "Please try again later", actions: [retryAction])
            case .success:
