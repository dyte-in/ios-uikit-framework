...
class  DyteStageActionButtonControlBar: DyteControlBarButton {
   ...
   public func addObserver() {
        ...
        self.selfListner.observeWebinarStageStatus { [weak self] status in
            guard let self = self else {return}
            self.stateMachine.forcedToSet(currentState: StageStatus.getStageStatus(status: status, mobileClient: self.mobileClient))
        }
