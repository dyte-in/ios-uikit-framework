...
class Item {
    var view: GridView<DyteParticipantTileView>
    var index: Int
    init(view: GridView<DyteParticipantTileView>, index: Int) {
        self.view = view
        self.index = index
    }
}
...
class PagingScrollView: UIScrollView {
    ...
    func reloadData() {
        if isDebugModeOn {
            print("Debug DyteUIKit | reloadData of Paging Scroll view")
        }
        resetScrollView()
...
extension PagingScrollView {
    ...
    @discardableResult  private func loadPage(index:Int) -> Bool {
        ...
        if isDebugModeOn {
            print("Debug DyteUIKit | Scroll Widht \(scrollWidth) height \(scrollHeight)")
        }
